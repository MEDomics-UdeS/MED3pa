...
max_depth_log = int(np.log2(datasets_manager.get_dataset_by_type('reference', True).get_features().shape[0]))
ipc_param_grid = {
...
ipc_cv = min(4, int(datasets_manager.get_dataset_by_type('reference', True).get_features().shape[0] / 2))
