import unittest
import numpy as np
from det3pa.datasets.manager import MaskedDataset
...
class TestMaskedDataset(unittest.TestCase):
    ...
    def test_refine(self):
        ...
        np.testing.assert_array_equal(self.dataset.features, self.features[mask])
    
    # tests that the sample method correctly samples a specified number of data points from the dataset.
    def test_sample(self):
        sampled_dataset = self.dataset.sample(2, seed=42)
    ...
    def test_set_pseudo_probabilities(self):
        ...
        np.testing.assert_array_equal(self.dataset.pseudo_labels, [1, 0, 1])
